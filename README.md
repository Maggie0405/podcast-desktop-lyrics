...
- 🤖 **自动打开逐字稿** —— 播新一集时自动替你按下播客的「听写文本」，触发系统下载逐字稿并进入实时模式
- 👻 **跟随播客显隐** —— 播客打开悬浮窗自动出现，播客退出自动隐藏（本程序静默驻留）
...
- **纯顺延（面板全程关闭）不够精确**。Apple 逐字稿是母版时间轴，实际发布音频经
  **动态广告插入 / 剪辑**后与之有逐段累积的偏移（可差几十秒），且断点信息本地不
  ...
  漂移，重新打开面板即可校正。
- ⚠️ **更新/重装本 App 后需要重新授权辅助功能**：App 是 adhoc 签名，每次构建签名指纹都会变，旧授权随之失效（设置里开关可能仍显示开启但不生效）。请在 系统设置›隐私与安全性›辅助功能 里把它**「−」移除再「+」重新添加**；App 启动时也会弹出系统引导。
...
1. 启动本程序（无 Dock 图标，悬浮窗常驻屏幕底部）
2. 打开 Apple 播客播放任意一集——程序会**自动帮你打开该集的「逐字稿」面板**并进入实时模式
3. 把播客窗口丢到后台即可；播客退出时悬浮窗自动隐藏
